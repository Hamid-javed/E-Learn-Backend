...
mongoose.connect("mongodb+srv://node-deploy:hamid123@node-deploy.gmwxivm.mongodb.net/?retryWrites=true&w=majority&appName=node-deploy");
...
const seedMentors = async () => {
    try {
        ...
        const newVid = new Video({ title: "web development", link: "https://www.youtube.com/embed/x4u1yp3Msao?si=C2qPCgP305bKLN2I" })
        const mn = await newVid.save()
        const videos = await Video.find()
        const video = videos[0]

        const newLessons = lessons.map((lesson) => {
            return lesson.map((less) => {
                return {
                    title: less.title,
                    desc: less.desc,
        ...
        for (let i = 0; i < courses.length; i++) {
            ...
            courses[i].data.description = courseDescriptions[i];
            ...
            courses[i].data.lessons = newLessons[i];
...
const all = async () => {
    ...
    //  const cou = await Mentor.findOne({_id: "66b719d63b5f779865910efb"}).populate("reviews.user").exec()
    //  let arr = []
    //  arr.push(cou)
